...
| 3 | Rendering and revision | "renderings please" after 1 | previews arrive one view per tick, then one rendering card; reject with a change → only the named piece re-rendered, a fresh card with the Revised row | brief ids, image count per view |
...
| 4.13.4 | 5 | passed (briefs #45/#46: first band frozen to the cent, shipping once, title whole); defect: twin labels "(men's wedding band)" twice → fixed locally; defect: the same band in rose gold was weighed again → twin rule, fixed locally | 7 Sep 2026 |
| 4.13.4 | 6, 7 | pending | |
