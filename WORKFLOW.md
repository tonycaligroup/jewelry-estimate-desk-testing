...
| A request to meet | Check the calendar for an existing meeting with this customer first. Build two or three fresh near-term times that are actually free inside declared windows. Send them to the owner as a booking approval. At Stage 3 the times may be offered to the customer while the owner decides; at Stage 1 or 2 nothing goes to the customer until the owner approves. The event is written and the customer is told "you're confirmed" only after the owner approves and the calendar write succeeds. An in-person event carries the configured scheduling location, or otherwise the shop address; a phone call carries no shop location. |
...
- Setup no longer asks retail or wholesale (4 September 2026); the profile
  ...
  to this document.
- First setup always asks the owner to choose the desk's operating mode:
  concierge (recommended, book the conversation and leave the estimate to the
  owner) or auto (gather details, price, and send only an approved estimate).
  Concierge is used without a choice only when the owner explicitly requests
  the default or declines to choose, never when the setup omits the question
  (13 September 2026).
- Fresh profiles carry unresolved setup choices for operating mode and the
  optional Google Sheets mirror. The setup profile command and readiness fail with
  the exact missing question until `desk.mode` is concierge/auto and
  `setup.sheet_mirror_choice` is create/adopt/skip. Sheet setup records create
  or adopt itself; skip is an explicit owner answer. Legacy configured profiles
  without the setup block remain compatible (13 September 2026).
- Owner delivery has two layers. Jewelry Desk questions, notices, and previews
  use the bound owner channel and Kolo's `notify-owner` preference; SMS requires
  an existing SMS one-to-one before it can be selected. Approval cards remain
  in Kolo's approval queue and are not routed by that preference. Fresh setup
  explains and configures both without promising SMS approval delivery
  (13 September 2026).
- Gmail raw-MIME sends add neither the Gmail web signature nor a display name
  on their own. Setup reads the exact matching send-as identity without writing
  Gmail, shows its display name and safely converted signature for owner
  confirmation, and stores them locally. Every customer email uses the
  confirmed `Name <mailbox>` and explicitly appends the confirmed signature;
  an empty Gmail identity falls back to `shop.name` and no invented signature
  (13 September 2026).
- A rejected appointment followed by an owner's replacement time always goes
  through the open `desk-answer` command, even if model resolution is degraded.
  The main session may not fabricate a card, accept approval of its own prose,
...
- Before any approval executes or any rejection is read, the watcher proves
  its Gmail credential with one read-only send-as request and requires the
  configured outbound mailbox to appear exactly once. The platform's
  `MATON_API_KEY` is read first, then `MATON_API_KEY_FILE`, then the fallback
  file, and nothing in the watcher replaces a credential. A failure is
  classified by HTTP status, never by message text: 401 `gateway_key_rejected`,
  403 with "No active connection" `integration_disconnected`, another 403
  `gateway_forbidden`, a 200 without the alias `outbound_mailbox_not_authorized`
  (the right account may have lost the alias, so it is not called a wrong
  account), a duplicate or malformed list `invalid_gateway_response`. The
  durable health record keeps the last success, the active failure, and the
  last failure; only a successful watcher preflight clears the active failure,
  and readiness and the doctor report it read-only. The owner hears once a
  day per class through the bound owner channel, with the one line to run, and
  ...
  prompted this was never found in any pod record afterwards, both keys
  were valid, and the desk had no way to record such a failure).
...
- With a meeting booked, a question that names a time is the meeting moving, whatever the words around it ('My mistake, I have the wrong day. Can we do Monday at 3pm?', 'Could we do Monday at 11 instead?'); a statement of the booked time ('See you Friday at 3pm!') is not. 'My mistake', 'wrong day', 'meant to say' are reschedule words. Live, 9 September 2026: the wrong-day reply got seven questions instead of the meeting card.
- The mirror's range URLs escape the tab name (a space in 'Cost sheet' is a control character to Python's HTTP client), and a failing sheet read never stops the sheet write. Live, 9 and 10 September 2026: every pull raised on the Cost sheet read, the push behind it never ran, and the Rates tab was never created, while the intake's rates sat in the profile.
- With a meeting booked, a bare day with reschedule words ('Sorry, I meant to say Tuesday', 'the 22nd works better') moves the meeting to that day at the booked time; the desk books it, never asks the owner. Live, 9 September 2026: the reply went to the owner as a stalled question with skip/ask again/handle myself.
- A reference to something from before is the whole family, not one phrase: a sentence that names a piece (or an estimate, a quote, a pair, an order) beside a cue to the past (remember, again, last time, before, earlier, another pair, one more, same as, like the ones I got, you made/sent/quoted, from you, years ago) counts; a sentence pointing at a photo, a website, an attachment, an heirloom or another jeweler does not. Any piece the desk then finds on file is said back and confirmed before pricing ('is that the piece you have in mind?'); nothing on file gets the welcome back and the reminder-or-photo ask. A new metal in their words drops the old metal's colour and karat ('everything the same but in platinum', not 'white platinum'). Live, 9 September 2026: 'remember those sapphire earrings?' was asked four questions with the pair on file.
- Judgement is the model's, the rule is the floor (the owner, 10 September 2026): the reading of each message also returns refers_to_earlier, the customer's own words when they point at a piece, estimate or design this shop already has with them, and a first email from a customer with an estimate on another thread is judged the same way before the owner is asked 'same piece or new'. Either signal, rule or model, sends the desk to the file. The judgement never rides on the record.
- An explicit remake or recreation from an attached photo is confirmed conservatively: broad piece and stone, a broad visible feature such as text, and the stated metal, matching the reference photo. The private reading may retain more detail for the bench, but inferred construction labels are not said back as customer facts. The photo settles metal color for that recreation and never supplies carat weight, karat, size, origin, or dimensions (13 September 2026).
