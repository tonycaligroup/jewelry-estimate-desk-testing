...
- Frontmatter version is `4.15.34` on master (the watcher proves the credential reaches an account authorized to send as the configured mailbox before any approval executes, one send-as request per tick; the platform environment key is read first, then MATON_API_KEY_FILE, then the fallback file, and nothing in the watcher replaces a credential; a 401, a disconnected integration, another 403, an unauthorized mailbox, and a malformed answer are recorded by class in a durable health record that only a successful watcher preflight clears, the owner hears once a day through the bound channel while cron stdout stays NO_REPLY, and readiness and the doctor report the record read-only; readiness runs in cron context and first activation requires that stamp; setup reads the exact Gmail send-as identity without writing Gmail, confirms it with the owner, and every raw MIME send carries the confirmed display name and explicit signature with safe fallbacks; setup separately configures the desk owner channel and Kolo's proactive `notify-owner` medium while stating that approval cards remain in Kolo's queue and are not routed by that preference; fresh-profile readiness now fails until concierge/auto, create/adopt/skip Sheets, and proactive-notification choices are recorded; first setup must explicitly ask the owner to choose concierge or auto mode and cannot silently default because the question was omitted; setup silently keeps the fixed retail-only mode and never asks retail, wholesale, trade, or both; an explicit remake or recreation from an attached photo is confirmed conservatively as the broad piece, stone, visible feature, and metal matching the reference; inferred construction labels are not repeated to the customer, and the photo settles metal color without supplying carat or other nonvisual facts; a concierge customer who declines the meeting and asks for a number is parked with nothing sent until the owner chooses acknowledge or handle myself; in-person calendar invitations carry the configured scheduling location or full shop address, while phone calls omit the shop location; monitor activation durably persists the exact verified cron binding, reconfiguration updates it, and business reset verifies it against active monitor state; customer-state reset recognizes and clears the desk's actual `.email-behavior-check-*` approval artifacts while still refusing unknown files; an appointment card is refused without its execute command; the session never revises a card by hand; the model resolved by served name from qwen, glm, haiku with a one-hour cache and a CLI guard; the desk's token file wins and install creates it; the watcher imports only the LiteLLM variables; fancy diamond colours are the stone's name; carried earring style; estimate wording by basis; the pair on file is found across singular and plural; an attached image settles the visual questions and is never asked for again; the watcher sources the platform env so cron runs direct; the same-sender question takes unrelated; the shadow behavior checker: a second no-tools read of every customer draft, verdicts on offer, booking and rendering cards, chat lines with the estimate id for price and un-carded drafts, evidence kept, no fallback; a reference to something from before is the whole family, with the model judging what no rule reads; any piece on file is said back and confirmed before pricing; a new metal drops the old colour and karat; the sheet's range URLs escape the tab name so pulls and pushes run again and the Rates tab is created; a bare day with reschedule words moves a booked meeting at the same time; a question naming a time after a booking is a reschedule; concierge books the call and stops, the owner sends the estimate; a repeat customer's piece on file is described back and confirmed, nothing on file gets a welcome back and a request for a reminder or photo; a missing rate is save, once, or a bare number that asks; a signature's number never makes a visit a call; courtesy notes after a booking send nothing; the customer-phrase rules in one table with live fixtures; the record schema; a piece on file outranks the reading's guesses; phone calls booked as calls with the number on the invitation; the cost sheet as the owner's workbench with read-back and drafts; the Rates tab, blank, with the pricing-notes intake and 'use 450 once'; the rules the jeweler's numbers switch on; Customers tab edits; carat ranges; an earlier estimate carried under new words; the cost sheet tab; bundled sends for cards born from one email; clock bounds in offers; the sheet skips non-inquiries; concierge mode by default: the call first, the owner's details in chat, one price card with renderings; the earring style question; times inside the days named; a bare day or a flat day-and-time after an offer; the vision confirmed from a photo; the center stone never an accent; the sender's own name; millimetre sizes; repeat pieces on file; the wider 'I don't know'; 50 scripts, manifest checked) (4.13.8 was the pod's republish of 4.13.7 after a stale publishing folder) (the team listing "jewelry-estimate-desk-testing"). Bump it with every publish.
- `tests/`, `ARCHITECTURE.md`, `WORKFLOW.md`, `RECORD-SCHEMA.md`, `KOLO-SKILL-PLAYBOOK.md`,
