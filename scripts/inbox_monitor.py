...
SCHEMA_VERSION = 1
ACTIVATION_STATES = {"prepared", "active"}
...
def validate_monitor_state(value: Any) -> dict[str, Any]:
    ...
    expected_hash = value.get("expected_cron_sha256")
    if not isinstance(expected_hash, str) or not expected_hash.startswith("sha256:"):
        raise ValueError("invalid expected_cron_sha256")
...
def prepare(root: Path, capabilities: Any, cron_config: Any) -> dict[str, Any]:
    ...
    if not isinstance(cron_config, dict) or not cron_config:
        raise ValueError("cron config must be a non-empty JSON object")
...
def activate(root: Path, cron_config: Any, activated_at_ms: int | None = None) -> dict[str, Any]:
    ...
    with setup_lock(root):
        ...
        if state["expected_cron_sha256"] != cron_hash:
