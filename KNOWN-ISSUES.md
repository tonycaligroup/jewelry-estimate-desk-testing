...
| 14 Sep UAT | Kolo reported Gmail connected while the watcher received HTTP 401; the pod probe later found both the private file's older key and the environment's newer key valid, the 401 gone on its own, and no record of the outage kept by the desk (the file-first loader had made an activation-time copy authoritative while the platform rotated the key underneath it) | The environment key is read first, then `MATON_API_KEY_FILE`, then the fallback file; one read-only send-as preflight runs before any approval and must find the outbound mailbox exactly once; failures are classified by status into a durable health record only the watcher clears; the owner hears once a day through the bound channel while cron stdout stays `NO_REPLY`; readiness runs in cron context and first activation requires its stamp; the watcher never replaces a credential | 4.15.34 |
