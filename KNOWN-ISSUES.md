...
| 14 Sep UAT | Kolo reported Gmail connected while the watcher repeatedly received HTTP 401 from a private token installed before a credential rotation | On 401 the watcher verifies the current platform credential against the exact configured mailbox before atomically refreshing; otherwise it preserves the old file and emits one concise daily repair notice | 4.15.33 |
