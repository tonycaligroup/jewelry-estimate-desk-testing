...
| 13 Sep UAT | After a rejected appointment, the session blamed model availability, fabricated a replacement offer, accepted approval in chat, and claimed success although the customer received nothing | Open `desk-answer` replies are now explicitly model-independent and must run through the delivered command; prose cannot substitute for a card or provider evidence, and concierge re-offers cannot restart intake | 4.15.32 |
| 14 Sep UAT | A Gmail HTTP 401 was reported while Kolo showed the integration connected. The pod probe found both the private file's older key (11 Sep) and the environment's newer key valid, and the tester's follow-up found no 401 in any pod record: all 508 cron runs ok, every "401" in the logs a digit substring of a timestamp. The report is uncorroborated; what the probe did establish is that the file-first loader had made an activation-time copy authoritative while the platform rotated the key underneath it, and that a failed tick on 4.15.30 left no record | The environment key is read first, then `MATON_API_KEY_FILE`, then the fallback file; one read-only send-as preflight runs before any approval and must find the outbound mailbox exactly once; failures are classified by status into a durable health record only the watcher clears; the owner hears once a day through the bound channel while cron stdout stays `NO_REPLY`; readiness runs in cron context and first activation requires its stamp; the watcher never replaces a credential | 4.15.34 |
